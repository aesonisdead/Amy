...
try {
if (!args[0]) return conn.reply(m.chat, `❀ Enter a country code to execute the command.`, m)
if (isNaN(args[0])) return conn.reply(m.chat, `ꕥ Enter a country code\nExample: ${usedPrefix + command} 212`, m)
const lol = args[0].replace(/[+]/g, '')
const ps = participants.map(u => u.id).filter(v => v !== conn.user.jid && v.startsWith(lol))
const bot = global.db.data.settings[conn.user.jid] || {}
if (ps.length === 0) return m.reply(`ꕥ There is no number here with the prefix +${lol}`)
const numeros = ps.map(v => '⭔ @' + v.replace(/@.+/, ''))
const delay = time => new Promise(res => setTimeout(res, time))
switch (command) {
case 'listanum': case 'listnum': {
conn.reply(m.chat, `❀ List of numbers with the prefix +${lol} who are in this group:\n\n` + numeros.join`\n`, m, { mentions: ps })
break
}
case 'kicknum': {
...
const error = `ꕥ @${user.split('@')[0]} has already been removed or has left the group.`
...
return m.reply(`⚠︎ An error occurred.`, m)
}}
break
}
}} catch (e) {
